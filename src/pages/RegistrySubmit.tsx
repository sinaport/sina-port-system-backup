...
// Engine ID is system-generated, never typed by a human (per feedback item 7).
// Prefix by economic engine flow + a unique suffix.
const FLOW_PREFIX: Record<string, string> = {
    Content: "CON",
    Funnel: "FUN",
    Resource: "RES",
    Inbound: "INB",
...
function generateEngineId(flow: string): string {
    const prefix = FLOW_PREFIX[flow] ?? "ENG";
    return `${prefix}-${Date.now().toString().slice(-6)}`;
}

// Title is system-composed from the flow + metric (per item 7: not a manual input).
// A richer, intelligence-generated title is the paid next phase.
function generateTitle(kind: RegistryKind, flow: string, metric: string): string {
    const m = metric || "metric";
    const f = flow || "flow";
    if (kind === "winner") return `${m} above target - ${f}`;
    return `${m} below target - ${f}`;
}
...
export function RegistrySubmit({ kind }: Props) {
    ...
    const buildPayload = (): Record<string, string | null> => {
        ...
        // Engine ID + title are generated by the system, not entered by the user (item 7).
        const engineId = generateEngineId(form.economic_engine_flow);
        const metricForTitle = kind === "winner" ? form.winning_metric : form.affected_metric;
        const autoTitle = generateTitle(kind, form.economic_engine_flow, metricForTitle);

        const common = {
            [idField]: id,
            economic_engine_flow: form.economic_engine_flow || null,
            economic_engine_id: engineId,
        ...
        <div className="max-w-3xl mx-auto px-4 py-6">
            ...
            <form onSubmit={handleSubmit} className="mt-6 space-y-6">
                ...
                <p className="text-xs text-zinc-500">
                    The record ID, Engine ID, and title are generated automatically when you submit - no need to fill those in.
                </p>
